        ...
        static async Task<string> AdvancedSentimentalAnalysis(string text)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                var requestBody = new
                {
                    ...
                    messages = new[]
                    {
                        new { role = "system", content = "You are an advances AI that anlayzes emotions in text. Your response must be in JSON format. Identiyf the sentiment scores(0-100%) for the following emotions: Joy, Sadness, Anger, Fear, Surprise and Neutral." },
